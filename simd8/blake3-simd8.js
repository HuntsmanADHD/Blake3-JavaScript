...
// ============================================================================
// BLAKE3 SIMD 8-wide - the fastest single-threaded build in this repo.
//
// Extends step 9 of the Fleek "Blake3: A JavaScript Optimization Case Study"
// blog: a compress8x is generated as WebAssembly SIMD *at load time* (no .wasm
// shipped, assembled from JS, compiled synchronously so hash() stays sync). It
// runs TWO interleaved 128-bit v128 streams = 8 chunks per call, for more
// instruction-level parallelism. Still 128-bit SIMD, still single-threaded.
//
// The fully-unrolled scalar compress (shared with blake3-ultra.js) handles small
// inputs, parent merges, and the tail; compress8x hashes 8 chunks in parallel
// for the bulk (~2.0 GiB/s at 256 KiB and up; throughput-bound, ~6% over 4-wide).
//
// Credits:
...
// ============================================================================
// compress8x: 8-way SIMD compression (two interleaved 4-lane v128 streams),
// generated as a WASM module on load. Stream A in v128 locals $0..$31, stream B
// in $32..$63; within each stream, $0..$15 = message words, $16..$31 = state.
// Each round emits stream A then stream B (8 chunks' worth) before advancing.
// Outputs the 8 chaining-value words per stream back over $16..$23 / $48..$55.
// ============================================================================
const { compress8x, wasmU32 } = (function () {
  ...
  const leb = (v) => { v |= 0; const r = []; for (;;) { const bb = v & 0x7f; v >>= 7; if ((v === 0 && (bb & 0x40) === 0) || (v === -1 && (bb & 0x40) !== 0)) { r.push(bb); return r; } r.push(bb | 0x80); } };
  put([
    0x00,0x61,0x73,0x6d, 0x01,0x00,0x00,0x00,
    0x01,0x04,0x01,0x60,0x00,0x00,
    0x02,0x0b,0x01,0x02,0x6a,0x73,0x03,0x6d,0x65,0x6d,0x02,0x00,0x01,
    0x03,0x02,0x01,0x00,
    0x07,0x0e,0x01,
    0x0a,0x63,0x6f,0x6d,0x70,0x72,0x65,0x73,0x73,0x38,0x78, 0x00,0x00,
    0x0a,0x00,0x00,0x00,0x00,0x00,
    0x01,
    0x00,0x00,0x00,0x00,0x00,
    0x01, 0x40, 0x7b,           // 64 x v128 locals
  ]);
  const body = n;
  for (let i = 0; i < 64; i++) put([0x41, ...leb(i * 16), 0xfd, 0, 4, 0, 0x21, i]);
  const M = [
  ...
  const gi = (a, b, c, d, dRot, bRot, m) => {
    const dShuf = dRot === 16 ? SHUF16 : SHUF8;
    put([0x20,a, 0x20,m, 0xfd,174,1, 0xfd,174,1, 0x22,a,
         0x20,d, 0xfd,81, 0x22,d, 0x20,d, 0xfd,0x0d, ...dShuf, 0x22,d,
         0x20,c, 0xfd,174,1, 0x22,c, 0x20,b, 0xfd,81, 0x22,b,
         0x41,bRot, 0xfd,173,1, 0x20,b, 0x41,32-bRot, 0xfd,171,1, 0xfd,80]);
  };
  const g = (a, b, c, d, m0, m1) => { put([0x20,b]); gi(a,b,c,d,16,12,m0); put([0x22,b]); gi(a,b,c,d,8,7,m1); put([0x21,b]); };
  for (let r = 0; r < 7; r++) {
    for (const sm of [[16,0],[48,32]]) {        // stream A then stream B
      const S = sm[0], Mb = sm[1]; let p = r*16;
      g(S+0,S+4,S+8,S+12, Mb+M[p++], Mb+M[p++]); g(S+1,S+5,S+9,S+13, Mb+M[p++], Mb+M[p++]);
      g(S+2,S+6,S+10,S+14, Mb+M[p++], Mb+M[p++]); g(S+3,S+7,S+11,S+15, Mb+M[p++], Mb+M[p++]);
      g(S+0,S+5,S+10,S+15, Mb+M[p++], Mb+M[p++]); g(S+1,S+6,S+11,S+12, Mb+M[p++], Mb+M[p++]);
      g(S+2,S+7,S+8,S+13, Mb+M[p++], Mb+M[p++]); g(S+3,S+4,S+9,S+14, Mb+M[p++], Mb+M[p++]);
    }
  }
  for (const S of [16, 48]) for (let i = 0; i < 8; i++) { const li = S + i; put([0x41, ...leb(li*16), 0x20,li, 0x20,li+8, 0xfd,81, 0xfd,11,4,0]); }
  put([0x0b]);
  const len = n - body + 3; writeLeb5(len, body - 8); writeLeb5(len + 6, body - 14);
  const memory = new WebAssembly.Memory({ initial: 1 });
  const inst = new WebAssembly.Instance(new WebAssembly.Module(code.subarray(0, n)), { js: { mem: memory } });
  return { compress8x: inst.exports.compress8x, wasmU32: new Uint32Array(memory.buffer) };
})();
...
function hash(input) {
  ...
  // ---- SIMD bulk: whole 8-chunk (8192-byte) groups, leaving >=1 byte for the tail ----
  const eightGroupEnd = (words !== null && length > 0) ? (((length - 1) / 8192) | 0) * 8192 : 0;
  if (offset < eightGroupEnd) {
    for (let i = 0; i < 4; i++) {
      wasmU32[96 + i] = 0x6a09e667; wasmU32[100 + i] = 0xbb67ae85; wasmU32[104 + i] = 0x3c6ef372; wasmU32[108 + i] = 0xa54ff53a; wasmU32[120 + i] = BLOCK_LEN;
      wasmU32[224 + i] = 0x6a09e667; wasmU32[228 + i] = 0xbb67ae85; wasmU32[232 + i] = 0x3c6ef372; wasmU32[236 + i] = 0xa54ff53a; wasmU32[248 + i] = BLOCK_LEN;
    }
  }
  while (offset < eightGroupEnd) {
    const gw = offset >> 2;
    for (let i = 0; i < 8; i++) {
      const c = IV[i], sa = (16 + i) * 4, sb = (48 + i) * 4;
      wasmU32[sa] = c; wasmU32[sa+1] = c; wasmU32[sa+2] = c; wasmU32[sa+3] = c;
      wasmU32[sb] = c; wasmU32[sb+1] = c; wasmU32[sb+2] = c; wasmU32[sb+3] = c;
    }
    for (let i = 0; i < 4; i++) {
      wasmU32[112 + i] = (chunkCounter + i) | 0;     wasmU32[116 + i] = ((chunkCounter + i) / 0x100000000) | 0;
      wasmU32[240 + i] = (chunkCounter + 4 + i) | 0; wasmU32[244 + i] = ((chunkCounter + 4 + i) / 0x100000000) | 0;
    }
    for (let blk = 0; blk < 16; blk++) {
      const base = gw + blk * 16;
      for (let wi = 0; wi < 16; wi++) {
        const b = base + wi, a4 = wi * 4, b4 = 128 + wi * 4;
        wasmU32[a4] = words[b]; wasmU32[a4+1] = words[b+256]; wasmU32[a4+2] = words[b+512]; wasmU32[a4+3] = words[b+768];
        wasmU32[b4] = words[b+1024]; wasmU32[b4+1] = words[b+1280]; wasmU32[b4+2] = words[b+1536]; wasmU32[b4+3] = words[b+1792];
      }
      ...
      wasmU32[252] = flags; wasmU32[253] = flags; wasmU32[254] = flags; wasmU32[255] = flags;
      compress8x();
    }
    for (let c = 0; c < 4; c++) { for (let k = 0; k < 8; k++) stack[stackPos + k] = wasmU32[64 + k*4 + c]; stackPos += 8; }
    for (let c = 0; c < 4; c++) { for (let k = 0; k < 8; k++) stack[stackPos + k] = wasmU32[192 + k*4 + c]; stackPos += 8; }
    const p = stackPos;
    compress(IV, 0, stack, p-64, stack, p-64, 0, BLOCK_LEN, PARENT, true);
    compress(IV, 0, stack, p-48, stack, p-56, 0, BLOCK_LEN, PARENT, true);
    compress(IV, 0, stack, p-32, stack, p-48, 0, BLOCK_LEN, PARENT, true);
    compress(IV, 0, stack, p-16, stack, p-40, 0, BLOCK_LEN, PARENT, true);
    compress(IV, 0, stack, p-64, stack, p-64, 0, BLOCK_LEN, PARENT, true);
    compress(IV, 0, stack, p-48, stack, p-56, 0, BLOCK_LEN, PARENT, true);
    compress(IV, 0, stack, p-64, stack, p-64, 0, BLOCK_LEN, PARENT, true);
    stackPos = p - 56;
    chunkCounter += 8; offset += 8192;
    let groups = chunkCounter >> 3;
    while ((groups & 1) === 0) { stackPos -= 16; compress(IV, 0, stack, stackPos, stack, stackPos, 0, BLOCK_LEN, PARENT, true); stackPos += 8; groups >>= 1; }
  }
...
if (typeof require !== 'undefined' && require.main === module) {
  console.log('BLAKE3 SIMD 8-wide - WASM compress8x (generated on load, no .wasm shipped)');
  ...
  const sizes = [[1024, '1 KiB', 20000], [65536, '64 KiB', 3000],
                 ...
                 [104857600, '100 MiB', 5], [1073741824, '1 GiB', 2]];
  for (const [size, label, iters] of sizes) {
    // fill via copyWithin replication (fast memcpy, no double allocation at 1 GiB)
    const data = new Uint8Array(size);
    const seed = Math.min(size, 65536);
    for (let i = 0; i < seed; i++) data[i] = i & 0xff;
    for (let o = seed; o < size; o += seed) data.copyWithin(o, 0, Math.min(seed, size - o));
    const warm = size > 20 * 1024 * 1024 ? 1 : 5;
    for (let i = 0; i < warm; i++) hash(data);
